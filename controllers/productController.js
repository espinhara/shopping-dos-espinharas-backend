...
exports.createProduct = async (req, res) => {
  try {
    ...
    if (imageUrls.length > 4) {
      return res.status(400).json({ message: 'Você pode fazer upload de até 4 imagens' });
    }
  ...
  try {
    ...
    if (req.files) {
      ...
      for (const file of req.files) {
        ...
        newImageUrls.push(result.secure_url); // Guardar a URL segura da imagem
      }
